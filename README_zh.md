...
drawface(facedata)#输入单个面顶点数据，绘制面
drawtex(facedata)#输入单个贴图顶点数据，绘制贴图
...
场景包含line、face和tex属性，存储边/面/贴图数据。
...
sort_tex_avg([相机坐标x,相机坐标y,相机坐标z])#透视模式或正交模式下调整图层顺序，修改场景对象属性并返回调整后数据
...
sort_tex_cabin()#斜二侧模式下调整图层顺序，修改场景对象属性并返回调整后数据
...
sort_tex_isometric()#等距模式下调整图层顺序，修改场景对象属性并返回调整后数据
